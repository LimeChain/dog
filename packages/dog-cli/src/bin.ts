...
const getDogQuestions = (networkConstants, deployerWallet) => {
	return [
		{
			type: "input",
			name: "tokenName",
...
In addition any investor can sell back their tokens for portion of the current organisation worth.`,
	...
	const networkAnswers = await inquirer.prompt(getNetworkQuestion());
	...
	let privateKey = (await inquirer.prompt(getPrivateKey())).privateKey;
	if (privateKey.length === 64) {
		privateKey = `0x${privateKey}`;
	}
	const deployerWallet = new ethers.Wallet(privateKey, provider);
	const config = await inquirer.prompt(
		getDogQuestions(NetworkConstants.getInvestmentTokens(networkAnswers.network), deployerWallet));
	...
	console.log(emoji.emojify(`:dog: Bark! Starting the deployment process... :`));

	const deploymentInfo = await builder.deploy(deployerWallet);

	console.log(
		emoji.emojify(`:dog: :newspaper: The contract will have the address of ${deploymentInfo.contractAddress}`));
	const txHashLink =
		`https://${networkAnswers.network === "mainnet" ? "" : networkAnswers.network}.etherscan.com/tx/${deploymentInfo.transactionHash}`;

	console.log(emoji.emojify(`:dog: :pick: You can view the deployment transaction at ${chalk.blue(txHashLink)}`));

	console.log(emoji.emojify(`:dog: Now we wait :hourglass_flowing_sand:`));

	await deploymentInfo.wait();

	console.log(emoji.emojify(`:dog: :confetti_ball::confetti_ball: Your DOG was deployed successfully! :confetti_ball::confetti_ball:`));
